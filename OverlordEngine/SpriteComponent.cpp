...
void SpriteComponent::Draw(const GameContext& )
{
	...
	SpriteRenderer* pInstance = SpriteRenderer::GetInstance();
	DirectX::XMFLOAT2 position = DirectX::XMFLOAT2(GetGameObject()->GetTransform()->GetPosition().x, GetTransform()->GetPosition().y);
	DirectX::XMFLOAT2 scale = DirectX::XMFLOAT2(GetGameObject()->GetTransform()->GetScale().x, GetTransform()->GetScale().y);
	DirectX::XMFLOAT3 rotation = QuaternionToEuler(GetGameObject()->GetTransform()->GetRotation());
	pInstance->Draw(m_pTexture, position, m_Color, m_Pivot, scale, rotation.z, GetTransform()->GetPosition().z);
}
