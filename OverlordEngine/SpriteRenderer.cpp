...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if(!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		bd.ByteWidth = sizeof(SpriteVertex) * (uint32_t)m_Sprites.size();
		...
		HRESULT result = gameContext.pDevice->CreateBuffer(&bd, &initData, &m_pVertexBuffer);
		Logger::LogHResult(result, L"Buffer Creation Failed");
	}
	...
	gameContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &vertexBufferData);
	// use memcpy to copy all our sprites to the mapped resource
	memcpy(vertexBufferData.pData, m_Sprites.data(), m_Sprites.size() * sizeof(SpriteVertex));
