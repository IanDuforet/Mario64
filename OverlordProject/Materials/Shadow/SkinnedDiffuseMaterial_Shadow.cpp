...
void SkinnedDiffuseMaterial_Shadow::LoadEffectVariables()
{
	...
	{
		SkinnedDiffuseMaterial_Shadow::m_pDiffuseSRVvariable = GetEffect()->GetVariableByName("gDiffuseMap")->AsShaderResource();
	...
	{
		SkinnedDiffuseMaterial_Shadow::m_pShadowSRVvariable = GetEffect()->GetVariableByName("gShadowMap")->AsShaderResource();
	...
	{
		SkinnedDiffuseMaterial_Shadow::m_pLightDirectionVariable = GetEffect()->GetVariableByName("gLightDirection")->AsVector();
	...
	{
		SkinnedDiffuseMaterial_Shadow::m_pLightWVPvariable = GetEffect()->GetVariableByName("gWorldViewProj_Light")->AsMatrix();
	...
	{
		SkinnedDiffuseMaterial_Shadow::m_pBoneTransforms = GetEffect()->GetVariableByName("gBones")->AsMatrix();
		if (!m_pBoneTransforms->IsValid())
		{
			Logger::LogWarning(L"DiffuseMaterial::LoadEffectVariables() > \'gBones\' variable not found!");
...
void SkinnedDiffuseMaterial_Shadow::UpdateEffectVariables(const GameContext& gameContext, ModelComponent* pModelComponent)
{
	...
	if (m_pBoneTransforms)
	{
		m_pBoneTransforms->SetMatrixArray(&pModelComponent->GetAnimator()->GetBoneTransforms().begin()->_11, 0, pModelComponent->GetAnimator()->GetBoneTransforms().size());
	}
