...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col, float2x2 uvRotation)
{
	...
	tempTexCoord -= float2(0.5f,0.5f);
	tempTexCoord = mul(tempTexCoord, uvRotation);
	tempTexCoord += float2(0.5f,0.5f);
	object.TexCoord = tempTexCoord;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	topLeft = float3((-size/2), (size/2), 0.f);
	topRight = float3((size/2), (size/2), 0.f);
	bottomLeft = float3((-size/2), (-size/2), 0.f);
	bottomRight = float3((size/2), (-size/2), 0.f);
