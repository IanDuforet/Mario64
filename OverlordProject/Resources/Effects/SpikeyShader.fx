...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal, float2 texCoord)
{
	...
	object.Position = mul(float4(pos, 1),m_MatrixWorldViewProj);
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float3 basePoint, top, left, right, spikeNormal, basePointNormal;
	...
	basePointNormal = (vertices[0].Normal + vertices[1].Normal + vertices[2].Normal)/3;
