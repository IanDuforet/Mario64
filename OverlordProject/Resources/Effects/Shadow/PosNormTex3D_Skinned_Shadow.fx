...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			int current = input.BoneIndices[i];
			transformedPosition += mul(input.BoneWeights[i] * originalPosition, gBones[current]);
			transformedNormal += mul(input.BoneWeights[i] * input.normal, (float3x3)gBones[current]);
		}
	}
...
float2 texOffset(int u, int v)
{
	...
	float2 offsettedValue;
	offsettedValue.x = u / 1280.f;
	offsettedValue.y = v / 720.f;
...
float EvaluateShadowMap(float4 lpos)
{
	...
	if (lpos.x < -1.0f || lpos.x > 1.0f ||
		...
		lpos.z < 0.0f || lpos.z > 1.0f) return 0.3f;
