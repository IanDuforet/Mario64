...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			int current = input.indices[i];
			transformedPosition += mul(input.weights[i] * originalPosition, gBones[current]);
			transformedNormal += mul(input.weights[i] * input.normal, (float3x3)gBones[current]);
		}
	}
